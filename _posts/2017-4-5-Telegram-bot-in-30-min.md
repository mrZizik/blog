...
Для работы наш телеграм бот, который крутится на нашей машине, должен получать обновления от сервера Teleram. Это можно сделать двумя методами:
* Long polling. Метод, когда ваша программа в определенный промежуток времени опрашивает сервер об обновлениях.
* Web hook. Тут вы поднимаете веб сервер на своей машине, на который телеграм бот шлет обновления, если они есть.

Второй метод более популярен и считается надежднее, поэтому остановимся на нем.
...
Первым делом нам надо зарегистрировать своего бота у @BotFather. Этот бот отвечает за управление информацией о боте, описании, получени и обновление токенов для работы с ними. От него мы получим такую строку 
...
```
Это ключ, по которому севрера телеграм будут знать, что мы владельцы этого бота. Если вы его потеряете, вы можете получить новый у того же @BotFather.

Первый шаг сделали. Мы уже сейчас можем найти нашего бота в ТГ и писать ему, но он пока только оболочка и отвечать вам не будет. Ему нужен призрак.

## Покупка выделенного сервера на DO

Как и любой сайт и серверсайд программа, наш код должен быть запущен на машине с белым ip и выходом в интернет. Для такого случая мы арендуем машину у digitalocean.com[]
...
Выбираем желаему ОС. Я буду работать с Ubuntu. 
...
Вводим наш пароль и получаем полный контроль над сервером. Возможно вас попросят сменить пароль, поставьте посложнее.
...
По хорошему, нам надо отделать окружения для каждого из наших python проектов, чтобы их зависимости не начали конфликтовать между собой. Для этого воспользуемся virtualenv.
...
source bin/activate
```
Теперь у нас есть локальная копия python, pip и все python модули установленные внутри, останутся в этом окружении. Все модули устанавливаем строго внутри окружения.
...
Так же сосздадим папку для логов и сделаем ее владельцем себя.
...
Теперь нам необходимо настроить веб сервер, чтобы свои запросы он направлял в сокет uwsgi.
...
Давайте чуть усложним бота и добавим кнопку "Обновить".
